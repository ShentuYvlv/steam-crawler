...
状态：已实现基础版（单条生成、批量后台提交、草稿查询、失败记录）。
...
状态：已实现基础版（草稿编辑、人工确认发送、发送记录、失败记录、重新生成入口）。
...
状态：已实现基础版（手动后台同步、任务状态查询、定时配置保存；真实 Redis/arq worker 待部署阶段替换）。
...
状态：已实现基础版（已发送列表、删除请求记录和审计日志；不调用 Steam 删除）。
...
状态：已实现基础版（真实数据库统计、首页指标卡、近 14 天趋势）。
...
状态：已按方案 B 实现基础版（独立账号密码登录、JWT、管理员创建用户、RBAC 角色和管理员用户管理菜单）。
